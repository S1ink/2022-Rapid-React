...
void CargoPipeline::process(cv::Mat& io_frame) {
	...
	if(_red) {
		...
		if(_contours) {
			...
			cv::drawContours(io_frame, this->red.proc.contours, -1, ::markup_map[~this->red.B_CLR][1]);
		}
	} else {
	...
	if(_blue) {
		...
		if(_contours) {
			...
			cv::drawContours(io_frame, this->blue.proc.contours, -1, ::markup_map[~this->blue.B_CLR][1]);
		}
	} else {
